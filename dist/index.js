...
export default class Robogo {
    ...
    groupTypes = { read: 'readGroups', write: 'writeGroups' };
    guardTypes = { read: 'readGuards', write: 'writeGuards' };
    ...
    logger;
    upload;
    constructor({ mongooseConnection, schemaPathGlob, servicePathGlob = null, fileDir = null, maxFileCacheAge = 5000, maxImageSize = 800, createThumbnail = false, maxThumbnailSize = 200, fileReadMiddleware = null, fileUploadMiddleware = null, fileDeleteMiddleware = null, checkAccess = true, namespaces = [], accessGroups = [], adminGroups = null, showErrors = true, showWarnings = true, showLogs = true, }) {
    ...
    async init() {
        const roboModel = this.generateModel(this.mongooseConnection.model('RoboFile'));
    ...
    async processSchemas() {
        for (const schemaPathGlob of await glob(this.schemaPathGlob)) {
            const { default: mongooseModel } = await import(schemaPathGlob);
    ...
    generateModel(mongooseModel) {
        const modelName = mongooseModel.modelName;
        const roboModel = {
            model: mongooseModel,
            name: mongooseModel.schema.get('name'),
            namespaces: mongooseModel.schema.get('namespaces') || [],
    ...
    addAccessGroupsToModel(mongooseModel, roboModel) {
        for (const groupType of Object.values(this.groupTypes)) {
            roboModel[groupType] = mongooseModel.schema.get(groupType);
    ...
    validateGroups(source, model) {
        for (const groupType of Object.values(this.groupTypes)) {
            ...
            for (const group of source[groupType]) {
                ...
                else if (this.accessGroups[group].length && model.namespaces.length
    ...
    generateRoboField(model, schema, type) {
        const roboField = {
            key: type.path,
        ...
        const optionKeys = ['required', 'name', 'description', 'enum', 'autopopulate', 'default', 'readGroups', 'writeGroups', 'marked', 'hidden'];
    ...
    getRoboTypeFromSchemaType(type) {
    ...
    addEmbeddedProperties(model, schema, roboField) {
        ...
        const keysToCopy = ['name', 'description', 'ref', 'enum', 'default', 'autopopulate'];
    ...
    decycleField(field, visitedRefs = []) {
    ...
    generatePathSchema(modelName, field, prefix = '') {
    ...
    collectMinimalRequiredAccessesGroupSetsOfModels() {
        for (const modelName in this.decycledSchemas) {
            this.models[modelName].minimalRequriedAccessGroupSets = this.collectMinimalRequiredAccessGroupSets({
    ...
    collectMinimalRequiredAccessGroupSets(field) {
    ...
    wrapExpressMiddleware(middleware) {
        return (req, res, next) => {
            if (!middleware)
                return next();
    ...
    async handleImageUpload(file) {
        ...
        return this.mongooseConnection.model('RoboFile').create(roboFileData);
    }
    /**
     ...
     */
    async resizeImageTo(sourcePath, size, destinationPath) {
    ...
    async handleFileUpload(file) {
        ...
        return this.mongooseConnection.model('RoboFile').create({
            name: file.originalname,
            path: filePath,
            size: file.size,
            extension,
    ...
    async CRUDSRoute({ req, res, operation, mainPart, responsePart }) {
        ...
        const mode = ['C', 'U', 'D'].includes(operation) ? 'write' : 'read';
    ...
    async hasModelAccess(modelName, mode, req) {
        if (mode === 'read' && !req.checkReadAccess)
            return true;
        if (mode === 'write' && !req.checkWriteAccess)
            return true;
        const groupType = this.groupTypes[mode];
        if (!this.hasGroupAccess(this.models[modelName][groupType], req.accessGroups))
            return false;
    ...
    async removeDeclinedFieldsFromObject({ object, mode, req, guardResults, ...params }) {
        ...
        const checkGroupAccess = !('modelName' in params) || !this.hasEveryNeededAccessGroup(params.modelName, mode, req.accessGroups);
    ...
    async removeDeclinedFieldsFromObjectHelper({ object, req, field, mode, checkGroupAccess, guardResults }) {
        ...
        if (declined) {
            ...
            if (Array.isArray(object[field.key])) {
                const promises = object[field.key].map(obj => this.removeDeclinedFieldsFromObject({ ...fieldsOrModel, object: obj, mode, req, guardResults }));
                await Promise.all(promises);
            }
            else {
                await this.removeDeclinedFieldsFromObject({ ...fieldsOrModel, object: object[field.key], mode, req, guardResults });
            }
        }
    }
    ...
    hasEveryNeededAccessGroup(modelName, mode, accessGroups) {
    ...
    async isFieldDecliendByGuards(req, field, mode, guardResults = null) {
        const guardType = this.guardTypes[mode];
    ...
    async getAccesses(modelName, req) {
        ...
        const promises = this.accessTypes.map(async (mode) => {
            ...
            const [skipGroupAccessCheck, guardResults] = await Promise.all([
                this.hasEveryNeededAccessGroup(modelName, mode, req.accessGroups),
    ...
    async removeDeclinedFieldsFromSchema({ fields, req, mode, modelName, guardResults }) {
        if (!fields.length)
            return fields;
        const checkGroupAccess = !modelName || !this.hasEveryNeededAccessGroup(modelName, mode, req.accessGroups);
    ...
    generateRoutes() {
        ...
        router.get('/read/:model', (req, res) => {
            this.CRUDSRoute({
                ...
                responsePart: async (results) => {
                    if (req.checkReadAccess)
                        await this.removeDeclinedFields({ modelName: req.params.model, documents: results, mode: 'read', req });
                    res.send(results);
                },
            });
        });
        router.get('/get/:model/:id', (req, res) => {
            this.CRUDSRoute({
                ...
                responsePart: async (result) => {
                    if (req.checkReadAccess)
                        await this.removeDeclinedFieldsFromObject({ modelName: req.params.model, object: result, mode: 'read', req });
                    res.send(result);
                },
            });
        });
        ...
        // FILE routes
        if (this.fileDir) {
            router.use('/static', this.wrapExpressMiddleware(this.fileReadMiddleware), express.static(this.fileDir, { maxAge: this.maxFileCacheAge }));
            router.use('/static', (req, res) => {
                res.status(404).send('NOT FOUND');
            });
            router.post('/fileupload', this.wrapExpressMiddleware(this.fileUploadMiddleware), this.upload.single('file'), async (req, res) => {
                try {
            ...
            router.post('/fileclone/:id', this.wrapExpressMiddleware(this.fileUploadMiddleware), async (req, res) => {
                try {
                    const roboFile = await this.mongooseConnection.model('RoboFile').findOne({ _id: req.params.id }).lean();
                    if (!roboFile)
                    ...
                    const file = await this.mongooseConnection.model('RoboFile').create(roboFile);
                    res.send(file);
                }
                catch (err) {
                    res.status(500).send(err);
            ...
            router.delete('/filedelete/:id', this.wrapExpressMiddleware(this.fileDeleteMiddleware), async (req, res) => {
                try {
                    const file = await this.mongooseConnection.model('RoboFile').findOne({ _id: req.params.id }).lean();
                    if (!file)
                        throw new Error('UNKNOWN FILE');
                    // Remove the file
                    ...
                    await this.mongooseConnection.model('RoboFile').deleteOne({ _id: file._id });
                    res.send();
                }
                catch (err) {
    ...
    async generateTSDefintions({ type, output }) {
