...
class Robogo {
  /**
   * A callback function that will be called on every file operation of its type and can stop the execution of it
   * @callback FileMiddleware
   * @param {Request} [req] The requests express.js Request object
   * @returns {Promise<void>} If this Promise gets rejected, the handler will stop the execution and the provided error message will be sent back to the client
   */

  /**
   * @typedef ConstructorParams
   * @prop {string} SchemaDir Absolute path to a folder containing files that are exporting the models that should be handled by robogo.
   * @prop {string} [ServiceDir] Absolute path to the folder that is containing the service files.
   * @prop {string} [FileDir] Absolute path to the folder in which uploaded files should be stored.
   * @prop {string} [ServeStaticPath='/static'] Relative path through which files in the FileDir will be accessible.
   * @prop {number} [MaxFileCacheAge=5000] The time in milliseconds until if the same file is requested another time, it can be served from the cache memory.
   * @prop {number} [MaxImageSize=800] Uploaded images higher or wider than this number will be resized to this size.Even if an image is already smaller then the size specified it will be compressed to take up less space. If null is given as value no resizing or compressing will be done.
   * @prop {boolean} [CreateThumbnail=false] Indicates whether robogo should create a small sized version of the images that are uploaded or not.
   * @prop {number} [MaxThumbnailSize=200] If CreateThumbnail is true, it behaves the same way as MaxImageSize but for thumbnail images.
   * @prop {FileMiddleware} [FileReadMiddleware]
   * @prop {FileMiddleware} [FileUploadMiddleware]
   * @prop {FileMiddleware} [FileDeleteMiddleware]
   * @prop {boolean} [CheckAccess=true] Indicates whether robogo should use its access features. Disabling it will speed operations up when handling huge documents.
   * @prop {string[]} [Softwares=[]]
   * @prop {string[] | Record<string, string[]>} [AccessGroups=[]]
   * @prop {string[] | Record<string, string[]>} [AdminGroups=[]]
   * @prop {boolean} [ShowErrors=true] Indicates whether robogo should log error messages to the console.
   * @prop {boolean} [ShowLogs=true] Indicates whether robogo should log basic information to the console.
   * @prop {boolean} [ShowWarnings=true] Indicates whether robogo should log warning messages to the console.
   * @prop {string} [ScriptExtension='.js']
   * @prop {mongoose.Connection} [MongooseConnection] Only needed if the mongoose connection was created using mongoose.connect().
   */
  ...
  constructor({
    ...
    this.BaseDBString = String(MongooseConnection.db.databaseName)
    this.Models = {}
   ...
   */
  GenerateSchemas() {
    for (const schemaPath of this._GetFilesRecursievely(this.SchemaDir).flat(Number.POSITIVE_INFINITY)) {
      ...
      for (const groupType of Object.values(this.GroupTypes)) {
        ...
        if (this.AdminGroups) {
          ...
          else { this.Logger.LogIncorrectAdminGroups(this.AdminGroups, `processing the admin groups of the model '${modelName}'`) }
        }
  ...
  CollectHighestAccessesOfModels() {
    for (const modelName in this.DecycledSchemas) {
      const accessesOfModel = this.CollectHighestAccessesOfModel(modelName, {
      ...
      this.Models[modelName].highestAccesses = {
        read: accessesOfModel.read.map((ag) => {
          ag.delete(null)
          return [...ag]
        }),
        write: accessesOfModel.write.map((ag) => {
          ag.delete(null)
          return [...ag]
        }),
      }
  ...
  CollectHighestAccessesOfModel(modelName, field) {
    ...
    if (!field.subfields || field.ref === 'RoboFile') {
      return { // if the field is a 'leaf' then we return our accesses
        read: field.readGroups ? field.readGroups.map(a => new Set([a])) : [],
        write: field.writeGroups ? field.writeGroups.map(a => new Set([a])) : [],
      }
    }

    const resOfSubfields = field.subfields.map(f => this.CollectHighestAccessesOfModel(field.ref, f)) // we calculate the results of our subfields

    // we collect all the combinations of the needed access groups into an object
    const fieldReadGroups = field.readGroups ? field.readGroups.map(a => new Set([a])) : []
    this.mergeChildAccessGroups(fieldReadGroups, resOfSubfields, 'read')

    const fieldWriteGroups = field.writeGroups ? field.writeGroups.map(a => new Set([a])) : []
  ...
  mergeChildAccessGroups(fieldGroups, resOfSubfields, mode) {
    ...
    for (const [index, nodeAccess] of fieldGroups.entries()) { // a node read access-ei
      for (const child of resOfSubfields) { // childen's results
        if (child[mode].some(g => [...g].every(a => nodeAccess.has(a))))
          continue

        const copysNeeded = child[mode].length - 1
        for (let i = 0; i < copysNeeded; ++i)
          fieldGroups.splice(index, 0, new Set(nodeAccess))

        for (const [ind, accessGroup] of child[mode].entries()) { // access groups of child
          const target = fieldGroups[index + ind]
   ...
   */
  GetSearchKeys(schema, maxDepth = Number.POSITIVE_INFINITY) {
  ...
  GenerateSearchKeys(field, keys, maxDepth, prefix = '', depth = 0) {
    ...
    if (!['Object', 'Date'].includes(field.type) && !field.subfields) // fuse.js can not handle values that are not strings or numbers, so we don't collect those keys.
  ...
  GenerateObjFieldTree(currentFieldLevel, fieldPath, fieldDescriptor, modelName) {
    ...
    /** @type {string} */
    // @ts-expect-error - the following pop() always returns at least an empty string
    const lastKey = fieldKeys.pop() // this is the field that we have information about from mongoose

    if (['_id', '__v', '$'].includes(lastKey))
    ...
    for (const groupType of Object.values(this.GroupTypes)) {
      ...
      for (const group of field[groupType]) {
        if (!this.AccessGroups[group]) {
          ...
          this.Logger.LogIncorrectAccessGroupSoftwareInField(
            modelName,
            field.key,
            group,
            this.AccessGroups[group],
            this.Models[modelName].softwares, `processing the field '${modelName} -> ${field.key}'`,
          )
        }
      }
    }
   ...
   */
  GetFields(schema, maxDepth = Number.POSITIVE_INFINITY, depth = 0) {
    if (typeof schema === 'string')
      schema = (maxDepth === Number.POSITIVE_INFINITY ? this.DecycledSchemas : this.Schemas[this.BaseDBString])[schema] // if string was given, we get the schema descriptor
  ...
  CRUDSRoute(req, res, mainPart, responsePart, operation) {
    ...
    const mode = opeartionToMode(operation)
  ...
  async calculateGuardPreCache(req, fields, mode) {
    ...
    for (const guard of guards) {
      // @ts-expect-error - the next shift() call always returns something
  ...
  async RemoveDeclinedFieldsFromSchema(fields, req, mode) {
    ...
    for (const field of fields) {
      const promise = this.IsFieldDeclined({ req, field, mode, checkGroupAccess, guardPreCache })
        .then((declined) => {
          if (declined)
            return Promise.reject(new Error('Decliend'))
  ...
  getAccesses(model, req) {
    ...
    return Promise.all([
      ...
      .then(([[_canReadAllRequired, read], [canWriteAllRequired, write]]) => {
        accesses.fields = {}
...
/**
 * @param {'C' | 'R' | 'U' | 'D' | 'S'} operation
 * @returns {'read' | 'write' | undefined}
 */
function opeartionToMode(operation) {
  switch (operation) {
    case 'R': return 'write'
    case 'C': return 'write'
    case 'U': return 'write'
